...
public class AddStudentServlet extends HttpServlet {
    ...
    private AddStudentClass asc;
    @Override
    public void init()
    {
        try{
            asc = new AddStudentClass();
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(asc.addStudent(eno,name,email,pword,dept,sub1,sub2,sub3,status,con))
        {
            s.setAttribute("StudentAdded","Student Data Added Successfully");
            response.sendRedirect("WelcomeAdmin.jsp");
        }
        else
        {
            s.setAttribute("StudentError","Student Data was not Added");
            response.sendRedirect("WelcomeAdmin.jsp");
        }       
    }
