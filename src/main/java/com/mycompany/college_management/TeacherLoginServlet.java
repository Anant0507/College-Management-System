...
public class TeacherLoginServlet extends HttpServlet {
    ...
    private FacultyCredential fc;
    private HODCredential hc;
    ...
    public void init()
    {
        try {
            ...
            fc = new FacultyCredential();
            hc = new HODCredential();
        } catch (Exception e) {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if(post.equals("HOD"))
            {
                if(hc.checkCredential(eno, pword, con))
                {
                    s.setAttribute("HODEnroll",eno);
                    response.sendRedirect("WelcomeHOD.jsp");
                }
                else
                {
                    s.setAttribute("HODError","Invalid Credentials");
                    response.sendRedirect("Teacher_Login.jsp");
                }
            }
            else if(post.equals("Faculty"))
            {
                if(fc.checkCredential(eno, pword, con))
                {
